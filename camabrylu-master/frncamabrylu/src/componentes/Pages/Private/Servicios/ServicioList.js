...
export default class Products extends Component {
 ...
 loadMore(page){
   ...
   const uri = `/api/limpieza/servicios/${page}/${items}`;
   saxios.get(uri)
     .then(
       ({data})=>{
         console.log(data);
         const { servicios : apiItems, total} = data;
      ...
      return (
        <div className='listItem' key={item._id}>
          ...
          <span className='updateListItem'>
            <Link to={`/servicio/${item._id}`}>
              <IoIosInformationCircleOutline size='2em'/>
            </Link>
            <Link to={`/servicio/img/${item._id}`}>
              <IoIosImage size='2em' />
            </Link>
          </span>
        </div>);
  ...
  return (
    <Page pageTitle="Servicios" auth={this.props.auth}>
